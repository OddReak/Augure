 ...
 * jours. §11, post-livraison — devenu une feuille (comme le menu du lieu),
 * pas un écran séparé : demandé par l'utilisateur plutôt que la navigation
 * en place jusqu'ici (route `/jour/:date`, retirée avec elle, voir
 * DECISIONS.md). Réutilise la même requête que l'accueil
 * (`useCoordonneesActuelles`, même clé TanStack Query) — aucun second appel
 * réseau, comme avant.
 *
 ...
 * Les métriques Humidité/Pression/Indice UV/Point de rosée/Visibilité ne
 * viennent que de `/api/current` (§4.1, endpoint quotidien sans ces
 * champs) : elles ne s'affichent donc que pour le jour courant, pas les
 * jours suivants — un sous-ensemble honnête plutôt qu'une valeur inventée
 * (voir DECISIONS.md).
 */
export function DetailJour({ date, onFermer }: DetailJourProps) {
  ...
  if (!requete.data || !jour) {
    return (
      <Feuille titre="Jour" onFermer={onFermer}>
        <p className={styles.absente}>Ce jour n&rsquo;est plus dans la prévision. Fermez et retournez à l&rsquo;accueil.</p>
      </Feuille>
    );
  }
  ...
  return (
    <Feuille titre={libelleJourLong(date)} description={libelleDateLongue(date)} onFermer={onFermer}>
      {(fermerAnime) => (
        <div className={styles.plein}>
          ...
          <button type="button" className={styles.fermer} onClick={() => fermerAnime()}>
            Fermer
          </button>
        </div>
      )}
